...
ElasticKernel checkpoints arbitrary Python objects, but how it does so depends on the
library. CI continuously verifies the `record_event → checkpoint → load_checkpoint` round
trip for the libraries below and classifies each one:

- ✅ **Migrated** — serialized with dill (the cost-optimized path).
- ♻️ **Recomputed** — not serializable, but restored by re-running the cell (the fallback works).
- ❌ **Failed** — could not be restored, or the value did not survive.
