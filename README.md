...
CI verifies a **representative object from each library** below — one object type per
library, since a library exposes many types, so this is a sample rather than a
whole-library guarantee — and reports whether each restore path reproduces it.

Legend: ✅ restored correctly · ➖ not serializable, so Migrate is N/A and Recompute is used · ❌ failed.
